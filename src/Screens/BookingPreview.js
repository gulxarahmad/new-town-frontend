...
export default function BookingPreview(props) {
    ...
    useEffect(()=>{
        fetch(`${apiURL}/readbookings/${id}`).then(res=>res.json()).then(data=>{setBooking(data); setLoading(false)});
    },[id])
    if(loading){
     ...
     <>   
      ...
      <div style={{display:'flex', flexDirection:'row'}}>
            ...
            <div style={{djustifyContent:'center', textAlign:'center', marginLeft:20, marginRight:20}}>
                ...
                <div>
                    <p style={{fontWeight:'bold', fontSize:20}}>NAWA PIND</p>
                </div>
                <div>
      ...
      <div style={{display:"flex",flexDirection:"row"}}> <br/> <br/>
      ...
      <div style={{display:"flex",flexDirection:"row"}}>  <br/> <br/> 
        ...
        <div style={{display:"flex",flexDirection:"row"}}>  <br/> <br/>
        <div style={{ width:240}}>
        <p>Second Applicant Name: {booking.secondAppName?booking.secondAppName:"none"}</p>
        </div>   
        <div style={{marginLeft:40}}>
        <p>Second Applicant CNIC: {booking.secondAppCNIC?booking.secondAppCNIC:"none"}</p>
        </div> 
       </div> 
       ...
       <div style={{display:"flex",flexDirection:"row"}}>  <br/> <br/>
        <div style={{ width:220}}>
        <p>Total Cost of Plot: {numbFormtter.format(booking.totalPlotCost)}Rs.</p>
        </div>   
        <div style={{ width:200}}>
    ...
    <p> By Bank Draft/Pay Order: {booking.bankOrDraft}</p>
      ...
      <p>Dated: {moment(booking.date).format('L')}</p>
       ...
       <h3>Declaration</h3>
       <p>(1) I, hereby declare that I have read and understood the Terms and Conditions of booking a plot in the project and accept the same</p>
       <p>(2) I, further agree to pay regularly installments and dues and abide by all existing rules and regulations and those which may be prescribed by the company from time to time</p>
    <p>(3) I enclosed here with a sum of rupees {booking.downPayment} by By {booking.bankOrDraft} dated: {moment(booking.date).format('L')} drawn on: {moment(booking.drawnOn).format('L')} on account of booking of the above plot</p> <br/>
       



    <div style={{display:"flex",flexDirection:"row", marginTop:30}}>  <br/> <br/>
        <div style={{ width:200}}>
